...
\hl{Knowing that it is possible to classify perceived music stimuli from EEG, and there is an overlap in brain areas active during music perception and imagination, we could therefore examine} \ac{EEG} data collected while participants listen to melodies to learn about the neural responses during music perception, and determine which salient elements are to be expected during music imagination.
...
\hl{Machine learning is a method that produces algorithms that can learn from and make predictions about data.}
...
\hl{Through backpropagation and the use of stochastic gradient descent, the layers of the model are trained to optimize the outcome. 
