...
We therefore can examine \ac{EEG} data collected while participants listen to melodies to learn about the neural responses during music perception, and determine which salient elements are to be expected during music imagination.
Exploring EEG data during music perception can inform how we approach music imagination data, and the brain signals recorded while listening to music could serve as reference data for decoding music imagination. 
To use a \ac{BCI} effectively both the system and the user need to be trained. 
...
By using perception data to train a \ac{BCI} we can cut down on the amount of imagination training needed, which will reduce potential user fatigue.
...
In the retina cells respond to small regions of the visual field \cite{hubel_receptive_1963}. 
...
At each level, more information is combined, giving cells higher up in the processing stream an increasingly global view of the information collected by the retinal cells.
For example, when looking at a house information about edges and colour are processed at lower levels.
This information is then combined as it moves into higher levels that process information like shape.
...
To classify our music stimuli from EEG data we first tried an ERP analysis similar to that of Schaefer \etal\cite{schaefer_name_2011} to determine which piece of music a participant was listening to or imagining.
